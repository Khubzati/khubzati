...
class RoleSelectionBloc extends Bloc<RoleSelectionEvent, RoleSelectionState> {
  ...
  RoleSelectionBloc({required this.appPreferences})
  ...
  Future<void> _onSelectRole(
      SelectRole event, Emitter<RoleSelectionState> emit) async {
  ...
  Future<void> _onLoadSelectedRole(
      LoadSelectedRole event, Emitter<RoleSelectionState> emit) async {
    emit(RoleSelectionInProgress());
    try {
