...
class VendorDetailBloc extends Bloc<VendorDetailEvent, VendorDetailState> {
  ...
  VendorDetailBloc(/*{required this.vendorService}*/) : super(VendorDetailInitial()) {
    on<FetchVendorDetails>(_onFetchVendorDetails);
  }

  Future<void> _onFetchVendorDetails(FetchVendorDetails event, Emitter<VendorDetailState> emit) async {
    ...
    try {
      ...
      final vendorDescription = "The best bakery in town, serving fresh goods daily since 1990. We use only the finest ingredients.";
      ...
      final reviews = ["Great products!", "Loved the cake!", "Fresh and tasty."];
      ...
      emit(VendorDetailError('Failed to fetch vendor details: ${e.toString()}'));
    }
  }
}
