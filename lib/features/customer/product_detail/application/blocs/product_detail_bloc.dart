...
class ProductDetailBloc extends Bloc<ProductDetailEvent, ProductDetailState> {
  ...
  ProductDetailBloc(/*{required this.productService, required this.cartBloc}*/) : super(ProductDetailInitial()) {
    on<FetchProductDetails>(_onFetchProductDetails);
    on<AddToCart>(_onAddToCart);
  }

  Future<void> _onFetchProductDetails(FetchProductDetails event, Emitter<ProductDetailState> emit) async {
    ...
    try {
      ...
      final product = {
        ...
        "description": "A flaky, buttery croissant, perfect for breakfast or a snack. Made with high-quality ingredients and baked fresh daily.",
        "price": 5.99,
      ...
      emit(ProductDetailError('Failed to fetch product details: ${e.toString()}'));
    }
  }
  ...
  Future<void> _onAddToCart(AddToCart event, Emitter<ProductDetailState> emit) async {
    ...
    try {
      ...
      print("Product ${event.productName} (ID: ${event.productId}) added to cart with quantity ${event.quantity}.");
