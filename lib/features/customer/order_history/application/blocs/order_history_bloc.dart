...
class OrderHistoryBloc extends Bloc<OrderHistoryEvent, OrderHistoryState> {
  ...
  Future<void> _onFetchOrderHistory(FetchOrderHistory event, Emitter<OrderHistoryState> emit) async {
    ...
    try {
      ...
      emit(OrderHistoryLoaded(
        orders: orders,
        hasReachedMax: orders.length < 10 || orders.length >= pagination['total_count'],
  ...
  Future<void> _onLoadMoreOrders(LoadMoreOrders event, Emitter<OrderHistoryState> emit) async {
      ...
      if (!currentState.hasReachedMax) {
        try {
          ...
          final response = await orderHistoryService.getOrderHistory(
            page: nextPage,
            limit: 10,
            status: currentState.currentFilter != 'all' ? currentState.currentFilter : null,
          );
          ...
          if (moreOrders.isEmpty) {
            ...
            emit(currentState.copyWith(
              ...
              hasReachedMax: moreOrders.length < 10 || 
                  (currentState.orders.length + moreOrders.length) >= pagination['total_count'],
            ));
          }
        } catch (e) {
          emit(OrderHistoryError('Failed to load more orders: ${e.toString()}'));
        }
      }
    }
  }
  ...
  Future<void> _onFilterOrdersByStatus(FilterOrdersByStatus event, Emitter<OrderHistoryState> emit) async {
    ...
    try {
      ...
      emit(OrderHistoryLoaded(
        ...
        hasReachedMax: filteredOrders.length < 10 || filteredOrders.length >= pagination['total_count'],
  ...
  Future<void> _onViewOrderDetails(ViewOrderDetails event, Emitter<OrderHistoryState> emit) async {
    ...
    try {
      ...
      final orderDetails = await orderHistoryService.getOrderDetails(event.orderId);
  ...
  Future<void> _onReorderItems(ReorderItems event, Emitter<OrderHistoryState> emit) async {
  ...
  Future<void> _onSearchOrders(SearchOrders event, Emitter<OrderHistoryState> emit) async {
    emit(OrderHistoryLoading());
    try {
      ...
      emit(OrderHistoryLoaded(
        ...
        hasReachedMax: searchResults.length < 10 || searchResults.length >= pagination['total_count'],
