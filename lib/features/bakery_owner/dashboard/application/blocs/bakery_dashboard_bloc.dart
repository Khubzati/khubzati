...
class BakeryDashboardBloc extends Bloc<BakeryDashboardEvent, BakeryDashboardState> {
  ...
  Future<void> _onLoadBakeryDashboard(LoadBakeryDashboard event, Emitter<BakeryDashboardState> emit) async {
  ...
  Future<void> _onFetchBakeryStats(FetchBakeryStats event, Emitter<BakeryDashboardState> emit) async {
    if (state is BakeryDashboardLoaded) {
      ...
      try {
        ...
        final stats = await bakeryDashboardService.getSalesStatistics(period: event.timeRange);
  ...
  Future<void> _onFetchRecentOrders(FetchRecentOrders event, Emitter<BakeryDashboardState> emit) async {
    if (state is BakeryDashboardLoaded) {
      final currentState = state as BakeryDashboardLoaded;
      ...
      try {
        // Fetch recent orders
        final recentOrders = await bakeryDashboardService.getRecentOrders(limit: event.limit);

        emit(currentState.copyWith(
          recentOrders: recentOrders,
        ));
      } catch (e) {
        emit(BakeryDashboardError('Failed to fetch recent orders: ${e.toString()}'));
        // Revert to previous state after error
        emit(currentState);
      }
    }
  ...
  Future<void> _onFetchPopularProducts(FetchPopularProducts event, Emitter<BakeryDashboardState> emit) async {
    if (state is BakeryDashboardLoaded) {
      final currentState = state as BakeryDashboardLoaded;
      ...
      try {
        ...
        final popularProducts = await bakeryDashboardService.getTopSellingProducts(limit: event.limit);

        emit(currentState.copyWith(
          popularProducts: popularProducts,
        ));
      } catch (e) {
        emit(BakeryDashboardError('Failed to fetch popular products: ${e.toString()}'));
        // Revert to previous state after error
        emit(currentState);
      }
    }
  ...
  Future<void> _onUpdateBakeryStatus(UpdateBakeryStatus event, Emitter<BakeryDashboardState> emit) async {
    if (state is BakeryDashboardLoaded) {
      final currentState = state as BakeryDashboardLoaded;
      ...
      try {
        ...
        final message = event.isOpen ? 'Bakery is now open for orders' : 'Bakery is now closed';
        emit(BakeryStatusUpdateSuccess(isOpen: event.isOpen, message: message));
        
        // Update the main state
        emit(currentState.copyWith(isOpen: event.isOpen));
      } catch (e) {
        emit(BakeryDashboardError('Failed to update bakery status: ${e.toString()}'));
        // Revert to previous state after error
        emit(currentState);
      }
    }
